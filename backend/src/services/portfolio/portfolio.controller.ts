...
export const portfolioController = {
    create: asyncHandler(async (req: Request, res: Response) => {
        ...
        res.status(201).json(new ApiResponse(201, portfolio, "Portfolio created"));
    }),

    getAll: asyncHandler(async (req: Request, res: Response) => {
        const userId = (req as any).user.id;
        const portfolios = await portfolioService.getUserPortfolios(userId);
        res.status(200).json(new ApiResponse(200, portfolios, "Portfolios retrieved"));
    }),
    ...
    buy: asyncHandler(async (req: Request, res: Response) => {
        ...
        res.status(200).json(new ApiResponse(200, result, "Buy order executed"));
    }),
    ...
    getDetails: asyncHandler(async (req: Request, res: Response) => {
        ...
        res.status(200).json(new ApiResponse(200, summary, "Portfolio details with live prices"));
    })
};